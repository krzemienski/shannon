---
name: spec-workflow
description: Spec-driven development pipeline (research → requirements → design → tasks → implement). ALWAYS use when the user says "build a feature", "create a spec", "spec-driven development", "research phase", "generate requirements", "create design", "plan tasks", "implement spec", "triage a feature", "create an epic", "decompose a large feature", or invokes /shannon:spec. Sequential phase artifacts in specs/<name>/. Supports epic flow for large features (decompose into dependency-aware sub-specs).
triggers:
  - "build a feature"
  - "create a spec"
