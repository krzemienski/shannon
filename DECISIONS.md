...
### D10 — Legacy-consolidation maximal-migration edition → v1.2.0 (2026-06-02)

- **Context:** the legacy `shannon-framework/` fork was audited in full (15-agent complete read, evidence under `evidence/legacy-consolidation/forge-fullmig-20260602T005139/`). The complete read inverted the migration premise: the fork's **top-level** dirs are the target's *superseded V3-V7 ancestors* + dead V5.6 UI + a simulated orchestration prototype + a Python test suite; the fork's nested `v1/shannon/` is the target's **direct ancestor**; the async skills runner (`skills10x`) is in the **target**, not the fork.
- **User calls (2026-06-02):** (1) **full maximal migration** — port every genuinely-additive, Iron-Rule-legal artifact; (2) **fresh full re-approval** at a new Phase-3 gate (supersedes `plans/260601-1712-shannon-consolidation/APPROVAL-SIGNED.md`); (3) architecture **C3 augment** (target stays canonical base); (4) **retire-with-reason** for dead/fake/forbidden buckets; (5) **wave-execution skill + `deepplan-architect` agent**, **UI deferred to v0.2.0** (honors D7).
- **Landed:** `forge` command + `documentation-research` + `oracle-review` skills; `wave-execution` skill (Serena→evidence-tree); `deepplan-architect` agent; benchmark harness (`scripts/harness/drive-tests*.sh` + `benchmarks/` + fixtures, G1+G2); `plan --mode deep` wired to wave decomposition; dangling `plan-deepen` refs repaired; version → **1.2.0**.
- **Reversal trail (decision-guard):** **D2** (signed component reductions) reversed → maximal *value* migration; documented here. **APPROVAL-SIGNED.md (2026-06-01)** superseded by `evidence/legacy-consolidation/forge-fullmig-20260602T005139/synthesis/APPROVAL-SIGNED-2026-06-02.md`. **D5/D6/D7/D8/D9** remain consistent.
- **Retired-with-reason (Iron Rule honored):** legacy `orchestration/*.py` (simulated `asyncio.sleep(0.01)` tasks), the Python `pytest` suite (test files — forbidden by `block-fab-files` hook), the dead V5.6 `dashboard/`+`server/` (deferred to v0.2.0 per D7), and the superseded V3-V7 `modules/core/modes` ancestors (duplicate-older). Full ledger: `docs/migration-map.md`.
- **P5 skills10x close-out (2026-06-02):** v1.2.0 installed (`claude plugin marketplace update shannon-local` → live plugin reads working tree; `claude plugin details` confirms `shannon 1.2.0` with `wave-execution`/`oracle-review`/`documentation-research`/`forge`/`deepplan-architect`). Fixed a real harness bug in `scripts/skills10x/discover_skills.py` — it could not see directory-source working-tree plugins (skills at `cwd/skills/`, no copy under `~/.claude/plugins/`); added a working-tree-plugin scan branch. `wave-execution` then graded end-to-end with 3 real `claude -p` probes → **f1=0.5 baseline (INTERMITTENT)**. The grading machinery is proven on the new set; a trigger-tuning *improvement* pass (to lift f1≥0.90) is a separately-reviewable skill-content edit, recorded as recommended follow-up, not bundled into the consolidation. Evidence: `evidence/legacy-consolidation/forge-fullmig-20260602T005139/validation/P5-CLOSE-findings.md`.
